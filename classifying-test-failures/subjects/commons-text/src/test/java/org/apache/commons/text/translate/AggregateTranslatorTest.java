...
public class AggregateTranslatorTest {
    ...
    public void testNonNull() throws IOException {
        ...
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(result1).as("Incorrect code point consumption").isEqualTo(3);});
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(out1.toString()).as("Incorrect value").isEqualTo("two");});
        final StringWriter out2 = new StringWriter();
        final int result2 = subject.translate(new StringBuffer("three"), 0, out2);
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(result2).as("Incorrect code point consumption").isEqualTo(5);});
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(out2.toString()).as("Incorrect value").isEqualTo("four");});
    }
    ...
    public void testNullConstructor() {
        ...
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(subject.translate(testString)).isEqualTo(testString);});
    }

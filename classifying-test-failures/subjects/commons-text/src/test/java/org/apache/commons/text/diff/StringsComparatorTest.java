...
public class StringsComparatorTest {
    ...
    public void testExecution() {
        for (int i = 0; i < before.size(); ++i) {
            ...
            int finalI = i;
            ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(ev.getString()).isEqualTo(after.get(finalI));});
    ...
    public void testLength() {
        for (int i = 0; i < before.size(); ++i) {
            ...
            ProxyAssertions.customized.Customized.runAssertion(() -> {
                assertThat(comparator.getScript().getModifications()).isEqualTo(length[finalI]);
            });
        }
    }
    @Test
    public void testLongestCommonSubsequence() {
        for (int i = 0; i < before.size(); ++i) {
            ...
            int finalI = i;
            ProxyAssertions.customized.Customized.runAssertion(() -> {
                assertThat(comparator.getScript().getLCSLength()).isEqualTo(lcs[finalI]);
            });

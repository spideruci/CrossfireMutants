...
public class EntityArraysTest  {
        ...
        for (final CharSequence escapeKey : escapeMap.keySet()) {
            for (final CharSequence unescapeKey : unescapeMap.keySet()) {
                if (escapeKey == unescapeMap.get(unescapeKey)) {
                    ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(unescapeKey).isEqualTo(escapeMap.get(escapeKey));});
                }
            }
        }
    }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("src/main/java/" + packageDirectory
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.contains("new HashMap<>();")) {
                    ...
                    int finalMapDeclarationCounter = mapDeclarationCounter;
                    ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(mapValue.size()).isEqualTo(finalMapDeclarationCounter);});
                }
            }
        }
    }
    ...
    public void testForDuplicateDeclaredMapValuesAposMap() {
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(EntityArrays.APOS_ESCAPE.keySet()).hasSameSizeAs(
                EntityArrays.APOS_UNESCAPE.keySet());});
    }

    @Test
    public void testForDuplicateDeclaredMapValuesBasicMap() {
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(EntityArrays.BASIC_ESCAPE.keySet()).hasSameSizeAs(
                EntityArrays.BASIC_UNESCAPE.keySet());});
    }

    @Test
    public void testForDuplicateDeclaredMapValuesHtml40ExtendedMap() {
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(EntityArrays.HTML40_EXTENDED_ESCAPE.keySet()).hasSameSizeAs(
                EntityArrays.HTML40_EXTENDED_UNESCAPE.keySet());});
    }

    @Test
    public void testForDuplicateDeclaredMapValuesISO8859Map() {
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(EntityArrays.ISO8859_1_ESCAPE.keySet()).hasSameSizeAs(
                EntityArrays.ISO8859_1_UNESCAPE.keySet());});
    }

    @Test
    public void testForDuplicateDeclaredMapValuesJavaCtrlCharsMap() {
        ProxyAssertions.customized.Customized.runAssertion(() -> {assertThat(EntityArrays.JAVA_CTRL_CHARS_ESCAPE.keySet()).hasSameSizeAs(

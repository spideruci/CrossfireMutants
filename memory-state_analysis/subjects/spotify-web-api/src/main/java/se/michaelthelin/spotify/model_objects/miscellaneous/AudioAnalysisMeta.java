  ...
  public static final class JsonUtil extends AbstractModelObject.JsonUtil<AudioAnalysisMeta> {
    public AudioAnalysisMeta createModelObject(JsonObject jsonObject) {
      ...
      return new AudioAnalysisMeta.Builder()
        .setAnalysisTime(
          hasAndNotNull(jsonObject, "analysis_time") // mutation (17) negated conditional -2001573640
            ? jsonObject.get("analysis_time").getAsFloat()
            : null)
        .setAnalyzerVersion(
          hasAndNotNull(jsonObject, "analyzer_version") // mutation (16) negated conditional -2001572927
            ? jsonObject.get("analyzer_version").getAsString()
            : null)
        .setDetailedStatus(
          hasAndNotNull(jsonObject, "detailed_status") // mutation (15) negated conditional -2001572245
            ? jsonObject.get("detailed_status").getAsString()
            : null)
        .setInputProcess(
          hasAndNotNull(jsonObject, "input_process") // mutation (14) negated conditional -2001571563
            ? jsonObject.get("input_process").getAsString()
            : null)
        .setPlatform(
          hasAndNotNull(jsonObject, "platform") // mutation (13) negated conditional -2001570881
            ? jsonObject.get("platform").getAsString()
            : null)
        .setStatusCode(
          hasAndNotNull(jsonObject, "status_code") // mutation (11) negated conditional -2001570199
            ? jsonObject.get("status_code").getAsInt()
            : null)
        .setTimestamp(
          hasAndNotNull(jsonObject, "timestamp") // mutation (11) negated conditional -2001569486

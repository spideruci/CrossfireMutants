...
public class AudioAnalysisSegment extends AbstractModelObject {
  ...
  public static final class Builder extends AbstractModelObject.Builder {
    ...
    public AudioAnalysisSegment build() {
      return new AudioAnalysisSegment(this);// mutation (58) 572983363
    }
  }
  ...
  public static final class JsonUtil extends AbstractModelObject.JsonUtil<AudioAnalysisSegment> {
    public AudioAnalysisSegment createModelObject(JsonObject jsonObject) {
      if (jsonObject == null || jsonObject.isJsonNull()) { // mutation (9) and mutation (10) negated conditional -1899067308 -1899067401
        return null;
      }

      return new AudioAnalysisSegment.Builder() // mutation (1) -1749212862 return null; can be killed
        .setLoudnessEnd(
          hasAndNotNull(jsonObject, "loudness_end") // mutation (8) negated conditional -1899066750
            ? jsonObject.get("loudness_end").getAsFloat()
            : null)
        .setLoudnessMax(
          hasAndNotNull(jsonObject, "loudness_max") // mutation (7) negated conditional -1899066037
            ? jsonObject.get("loudness_max").getAsFloat()
            : null)
        .setLoudnessMaxTime(
          hasAndNotNull(jsonObject, "loudness_max_time") // mutation (6) negated conditional -1899065324
            ? jsonObject.get("loudness_max_time").getAsFloat()
            : null)
        .setLoudnessStart(
          hasAndNotNull(jsonObject, "loudness_start") // mutation (5) negated conditional -1899064611
            ? jsonObject.get("loudness_start").getAsFloat()
            : null)
        .setMeasure(
          new AudioAnalysisMeasure.JsonUtil().createModelObject(jsonObject))
        .setPitches(
          hasAndNotNull(jsonObject, "pitches") // mutation (4) negated conditional '-1899063588
            ? new Gson().fromJson(jsonObject.getAsJsonArray("pitches"), float[].class)
            : null)
        .setTimbre(
          hasAndNotNull(jsonObject, "timbre") // mutation (3) negated conditional -1899062751

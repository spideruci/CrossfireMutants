...
import static stateprobe.pitest.ProblemRecorder.*;
...
public class Pitest {
  ...
  private void executeTests(final Container container,
    ...
    for (final TestUnit unit : testUnits) {
      System.err.println(unit.getDescription().getQualifiedName());
      //record doing which test?
      ProblemRecorder.setWhichTest(i);
      ProblemRecorder.initializeMRNMRProbes();
      ProblemRecorder.clearFile("target/stateInfo.txt");
      long t1 = System.nanoTime();
      final List<TestResult> results = container.execute(unit);
      long t2 = System.nanoTime();
      processResults(results);
      collectProbeInfo();
      organizeStates(i, unit.getDescription().getQualifiedName(),t2 - t1,testUnits.size());
      i = i + 1;
      System.gc();
    }
  }

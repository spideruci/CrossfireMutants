...
public class AbstractPitMojo extends AbstractMojo {
  ...
  public final void execute() throws MojoExecutionException,
    ...
    if (shouldRun.shouldRun()) {
      //create the file that record the mutated method;
      ProblemRecorder.addOneLine("ha ha hahaha", "target/mutatedDesc.txt");
      this.getLog().info("create target/mutatedDesc.txt to record mutated method name");
      //create the file that record the times that mutations are executed.
      this.getLog().info("this is an experimental version");
      this.getLog().info("Root dir is : " + projectBase);
      ProblemRecorder.createDirectory("target/staticFields");
      ProblemRecorder.clearFile("target/staticFields.txt");
      ProblemRecorder.clearFile("target/globalData.txt");
      ProblemRecorder.setInitialState(); //set GlobalStates.txt
